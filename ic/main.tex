...
\newcommand{\sectionBox}[1]{
    ...
    \colorbox{sectionBg}{\parbox{\dimexpr\linewidth-4\fboxsep\relax}{\MakeUppercase{#1}}}
...
\titleformat{\section}      % comando que altera o formato da seção
    [block]                 % comando que altera o formato da seção
    {\bfseries\normalsize}  % formatação do texto (negrito + tamanho)
    {}                      % formato do número (vazio = sem número)
    {0pt}                   % espaçamento entre o número e o título
    {
        \sectionBox
    }
...
% Compile with xelatex -> bibtex -> xelatex x2
